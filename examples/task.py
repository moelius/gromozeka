...
from gromozeka import Gromozeka, BrokerPointType, ThreadWorker, ProcessWorker, task
...
if __name__ == '__main__':
    ...
    broker_point_first = BrokerPointType(exchange='first_exchange', exchange_type='direct', queue='first_queue',
                                         routing_key='first')
    broker_point_second = BrokerPointType(exchange='second_exchange', exchange_type='direct', queue='second_queue',
                                          routing_key='second')
    ...
    app.register_task(task=test_func_one(), broker_point=broker_point_first, worker_class=ThreadWorker, max_workers=10)
    app.register_task(task=test_func_second(), broker_point=broker_point_second, worker_class=ProcessWorker,
                      max_workers=10)
